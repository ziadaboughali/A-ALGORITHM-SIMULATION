...
def adaptiveA(self, grid):
    ...
    #arbitrary weight value for adaptive a* (value must be >=1)
    weight = 1.05
    ...
    while open_list:
        ...
        for i, j in neighbors:
            ...
            self.h = self.manhattan(successor, self.goal) * weight  # Update h with the weight
            ...
            if heuristic_g < state_g.get(successor, 0) or successor not in [i[1] for i in open_list]:
                ...
                state_f[successor] = heuristic_g + self.manhattan(successor, self.goal) * weight # Update f with the weight
